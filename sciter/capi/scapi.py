...
def SciterAPI():
    ...
    # 1. Tự động kiểm tra DLL nằm trong chính thư mục package sciter/
    capi_dir = os.path.dirname(os.path.abspath(__file__))
    package_dir = os.path.dirname(capi_dir)
    
    local_dll_name = None
    if SCITER_WIN:
        local_dll_name = "sciter64.dll" if sys.maxsize > 2**32 else "sciter32.dll"
    elif SCITER_OSX:
        local_dll_name = SCITER_DLL_NAME + ".dylib"
    elif SCITER_LNX:
        local_dll_name = SCITER_DLL_NAME + ".so"
        
    if local_dll_name:
        local_dll_path = os.path.join(package_dir, local_dll_name)
        if os.path.exists(local_dll_path):
            try:
                if SCITER_WIN:
                    if hasattr(os, 'add_dll_directory'):
                        os.add_dll_directory(package_dir)
                    scdll = ctypes.WinDLL(local_dll_path)
                else:
                    RTLD_LAZY = 1
                    scdll = ctypes.CDLL(local_dll_path, ctypes.RTLD_LOCAL | RTLD_LAZY)
            except OSError as e:
                errors.append("Tried local DLL '%s' but failed: %s" % (local_dll_path, str(e)))

    # 2. Nếu không có DLL cục bộ, fallback về cơ chế mặc định của pysciter
    if not scdll:
        if SCITER_WIN:
